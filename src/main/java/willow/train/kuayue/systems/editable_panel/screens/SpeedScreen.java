...
public class SpeedScreen extends CustomScreen<EditablePanelEditMenu, EditablePanelEntity> {
    ...
    public void init() {
        ...
        contentLabel.setOnCLick((widget, mouseX, mouseY) -> {
            editBar.setPosition(contentLabel.getX() - (editBar.getWidth() - contentLabel.getWidth()) / 2, contentLabel.getY() - 20);
            editBar.visible = true;
        });
        contentLabel.setColor(nbt.getInt("color"));
        int q = (Minecraft.getInstance().screen.width - 105) / 2;
        colorSelector.getColorBtn().setPos(q + 5, contentLabel.getY() + 20);
        colorSelector.getTemplateBtn().setPos(q + 25, contentLabel.getY() + 20);
        offsetEditor.getEditorBtn().setPos(q + 45, contentLabel.getY() + 20);

        confirmBtn = new ImageButton(acceptBtnImage, q + 85, contentLabel.getY() + 20,
        ...
        cancelBtn = new ImageButton(cancelBtnImage, q + 65, contentLabel.getY() + 20,
                16, 16, Component.empty(), b -> {
